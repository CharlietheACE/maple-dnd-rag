...
test("uses current vector-store file attributes and flat static chunking shape", async () => {
  ...
  assert.deepEqual(JSON.parse(String(request.init?.body)), {
    ...
    chunking_strategy: { type: "static", max_chunk_size_tokens: 800, chunk_overlap_tokens: 400 },
  });
